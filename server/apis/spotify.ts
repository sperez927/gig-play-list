...
import { mockArtists, mockTracks } from "../mockDb";

export const getArtistTracks = async (artistName: string) => {
  const search = (artistName || '').toLowerCase().trim();
  // Try to find artist by partial match (include-search)
  const artist = mockArtists.find(a => a.name.toLowerCase().includes(search));
  if (!artist) throw { status: 404, data: "Artist not found" };
  const tracks = mockTracks[artist.name as keyof typeof mockTracks] || [];
  return {
    ...artist,
    tracks,
  };
